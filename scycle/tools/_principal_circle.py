...
def principal_circle (adata: AnnData, n_nodes: int=30, verbose: bool=True):
    ...
    node_read_counts = [np.mean(total_counts[np.where(partition==i)[0]]) 
...
def _get_gr_coords (adata):
    ...
    node_ord_df = pd.DataFrame({'nid': node_order, 
...
def _get_edge_coords(point_coords, edges):
    ...
    while (pid != start_pid):
        ...
        query_res = edges.query('e1 == ' + str(pid) + '| e2 ==' + str(pid)).to_numpy().flatten() 
    ...
    edge_coords = edge_coords.merge(point_coords, how = 'left', left_on='nid', right_on='nid').sort_values('eid')
    node_order = s_order
