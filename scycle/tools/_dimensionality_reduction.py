...
def dimensionality_reduction(adata: AnnData, method: str='pcaCCgenes', n_comps: int=30, 
            decomposition = None,
    ...
    if 'scycle' not in adata.uns:
        raise Exception(('Data needs to be pre-processed by `pp.prep_pooling`' +
                        'or `pp.prep_simple`, before dimensionality reduction'))
        
    if decomposition is not None: 
        ccomps = range(decomposition.n_components) if choose_comps is None else choose_comps
        genes = np.array(adata.var.index) if decomposition_features is None else decomposition_features
        ...
        if method == 'pca': dimred_res = _dimRed_pca(adata, n_comps = n_comps, verbose = verbose)
        elif method == 'pcaCCgenes': dimred_res = _dimRed_pca(adata_cc, n_comps = n_comps, verbose = verbose)
        elif method == 'ica': dimred_res = _dimRed_ica(adata, n_comps = n_comps, max_iter = max_iter, seed = seed, verbose = verbose)
        elif method =='icaCCgenes': dimred_res = _dimRed_ica(adata_cc, n_comps = n_comps, max_iter = max_iter, seed = seed, verbose = verbose)
        ...
        elif method == 'CCgenes': dimred_res = _dimRed_CCgenes(adata, verbose = verbose)
        else:
            raise Exception(('Not one of the supported methods.\n' + 
...
def _adata_CCgenes (adata):
    ...
    flat_sigs = [item for sublist in [v for k, v in cc_sigs.items()] for item in sublist]
...
def _dimRed_pca(adata, n_comps, verbose = False):
        if verbose: print('Dimensionality reduction using PCA...')
...
def _dimRed_CCgenes(adata, verbose = False):
    if verbose: print('-- Dimensionality reduction using G1 and G2/M signatures...')
